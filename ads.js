...
function initPurchases() {
  try {
    ...
    // Non-consumable: bought once, kept forever, restorable. Must match the
    // type set in App Store Connect.
...
async function boot() {
  ...
  /* Consent is requested on the FIRST launch, every launch, for everyone.
     An earlier version deferred this to the second launch to improve opt-in
     rates. That meant a short first session saw no prompts and no ads while
     a returning user saw both, which is indistinguishable from hiding
     monetisation from review. Do not reintroduce it. */
  await establishConsent();
